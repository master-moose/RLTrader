...
class CryptoTradingEnv(gym.Env):
    ...
    def _close_position(self, price):
        ...
        position_value = abs(self.position) * executed_price
    ...
    def _update_equity(self, current_price):
        ...
        # Calculate return
        if self.returns:
            prev_equity = self.returns[-1][1]
            ret = (self.equity - prev_equity) / prev_equity
        else:
            ret = 0
        
        self.returns.append((self.current_step, self.equity, ret))
    ...
    def _get_observation(self):
        ...
        normalized_position = self.position * current_price / self.initial_balance
    ...
    def _calculate_reward(self, prev_equity):
        """
        ...
        if self.reward_function == 'pnl':
            # Simple P&L reward
            reward = (self.equity - prev_equity) / self.initial_balance
    ...
    def render(self, mode='human'):
        ...
        if mode == 'human':
            ...
            position_info = f"Position: {self.position:.4f} @ {self.position_price:.2f}" if self.position != 0 else "No position"
            return (
