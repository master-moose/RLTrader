...
EXCHANGE_ID = 'binance'
SYMBOL = 'BTC/USDT'
# How many levels of bids/asks to fetch
# (Binance allows 5, 10, 20, 50, 100, 500, 1000)
# Higher limit = more data per snapshot but potentially slower fetch
LOB_LIMIT = 100
...
def collect_lob_data():
    ...
    try:
        exchange = getattr(ccxt, EXCHANGE_ID)({
            ...
            # Default Binance rate limit, enableRateLimit handles this
            # 'rateLimit': 1200,
        ...
        if SYMBOL not in exchange.markets:
            logging.error(
                f"Symbol {SYMBOL} not found on {EXCHANGE_ID}. Exiting.")
            return
    ...
    while True:
        try:
            ...
            # Alternatives: Flatten lists, use multi-index, separate tables.
            bids_list = order_book.get('bids', [])
            asks_list = order_book.get('asks', [])
            data_to_store = pd.DataFrame({
                'timestamp_utc': [fetch_timestamp_utc],
                # Exchange LOB timestamp
                'lob_timestamp_ms': [order_book.get('timestamp')],
            ...
            data_to_store['timestamp_utc'] = pd.to_datetime(
                data_to_store['timestamp_utc'])
            ...
            with pd.HDFStore(current_file, mode='a') as store:
                store.append(HDF_KEY, data_to_store, format='table',
                             ...
                             data_columns=['timestamp_utc'],
                             # Pre-allocate space for strings
                             min_itemsize={'bids': 4096, 'asks': 4096})

            logging.info(
                f"Stored LOB snapshot for {SYMBOL} at {fetch_timestamp_utc}")

            # Calculate sleep time, ensuring we adhere to the interval
            elapsed_time = time.time() - start_time
            sleep_time = max(0, FETCH_INTERVAL_SECONDS - elapsed_time)
            logging.debug(
                f"Fetch took {elapsed_time:.3f}s, sleeping for {sleep_time:.3f}s")
            time.sleep(sleep_time)

        except ccxt.RateLimitExceeded as e:
            logging.warning(
                f"Rate limit exceeded: {e}. Retrying after delay...")
            ...
            retries += 1
            logging.error(
                f"Network/Exchange error: {e}. Retry {retries}/{MAX_RETRIES}")
            ...
            logging.exception(
                f"An unexpected error occurred: {e}. Attempting to continue...")
