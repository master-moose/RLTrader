...
def check_resources(
    ...
    if memory.percent > warning_threshold * 100:
        ...
        if garbage_collect:
            ...
            if logger:
                logger.info(f"Garbage collection freed {memory_freed:.2f}GB of memory")
...
def save_config(config: Dict[str, Any], log_dir: str, filename: str = "config.json") -> None:
    """
...
def create_evaluation_plots(
    ...
    axes[0].plot(steps, portfolio_values, label="Portfolio Value", color="blue", linewidth=2)
    ...
    if len(portfolio_values) > 1:
        ...
        ax_returns.plot(steps[1:], cumulative_returns, label="Cumulative Return", 
    ...
    if actions:
        ...
        for action in sorted(set(actions)):
            ...
            axes[1].scatter(
                ...
                label=f"{action_labels.get(action, action)} ({action_counts.get(action, 0)})",
        ...
        axes[1].set_yticklabels([action_labels.get(a, a) for a in sorted(set(actions))])
        axes[1].legend(loc="upper right")
        axes[1].grid(True)
    
    # Plot rewards or positions in the third subplot
    if rewards:
        axes[2].plot(steps[:len(rewards)], rewards, label="Reward", color="purple", linewidth=1.5)
        ...
        ax_cum_rewards.plot(steps[:len(rewards)], cum_rewards, label="Cumulative Reward", 
    ...
    if positions:
        if len(rewards) == 0:  # Only if we haven't plotted rewards
            axes[2].plot(steps[:len(positions)], positions, label="Position", color="brown", linewidth=1.5)
