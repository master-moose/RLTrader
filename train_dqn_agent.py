...
class PatchedStockTradingEnv(BaseStockTradingEnv):
    ...
    def step(self, actions):
        ...
        if self.post_trade_frozen and self.current_step < self.frozen_until_step:
            if isinstance(actions, np.ndarray) and len(actions) > 0:
                actions = np.ones_like(actions)  # Force hold (1)
            else:
                actions = 1  # Hold action
        ...
        # CRITICAL: Strictly enforce cooldown and always consider non-hold actions as trade attempts
        if steps_since_last_trade < self.trade_cooldown:
            # Detect if action is not hold (meaning the agent is trying to trade)
            if isinstance(actions, np.ndarray) and len(actions) > 0:
                # Check if any action is not a hold
                is_hold_action = np.all(actions == 1)
                attempted_trade_during_cooldown = not is_hold_action
            elif isinstance(actions, (int, float, np.int64, np.float64)):
            ...
            if attempted_trade_during_cooldown:
                ...
                if self.consecutive_attempts > 20:
                    logger.error(f"CRITICAL: Excessive consecutive trade attempts during cooldown: {self.consecutive_attempts}")
                    # Add even more severe penalties for persistent attempts
                    if self.consecutive_attempts % 10 == 0:
                        logger.error(f"Trading agent attempting to bypass cooldown mechanisms: {self.consecutive_attempts} consecutive attempts")
                ...
                # If actions were modified, log the change
                if self.debug_mode:
                    if isinstance(original_actions, np.ndarray) and len(original_actions) > 0:
                        logger.debug(f"Modified actions from {original_actions} to {actions} due to cooldown")
                    else:
                        logger.debug(f"Modified action from {original_actions} to {actions} due to cooldown")
        else:
            # Reset consecutive attempts counter when not in cooldown
            self.consecutive_attempts = 0
        
        # Call parent step method with potentially modified actions
        ...
        if trade_occurred:
            ...
            if force_cooldown:
                ...
                logger.error(f"CRITICAL: Trade somehow executed during cooldown! Applying extreme penalty at step {self.current_step}")
        ...
        info['enforced_hold'] = steps_since_last_trade < self.trade_cooldown or self.post_trade_frozen
    ...
    def reset(self, seed=None, options=None):
        """
        Reset the environment to initial state with enhanced features.
        ...
        # Reset current step and trading tracking variables
        self.current_step = 0
        self._last_trade_step = None
        self._last_trade_price = {}
        self.info_buffer = {}
        
        # Reset action and position histories
        self.action_history = []
        self.position_history = []
        self.last_positions = []
        ...
        # Reset frozen period status
        self.post_trade_frozen = False
        self.frozen_until_step = 0
        
        # Reset same price trade tracking
        self.last_trade_prices = {}
        self.last_position_values = {}
        self.same_price_trade_counter = 0
        
        # Log reset
        logger.info(f"Reset PatchedStockTradingEnv with {self.stock_dim} assets")
    ...
    def _calculate_reward(self, begin_total_asset, end_total_asset, attempted_trade_during_cooldown=False):
        ...
        Apply penalties for attempted trading during cooldown.
        """
        # Base reward is the change in total asset value
        reward = (end_total_asset - begin_total_asset) * self.reward_scaling
        
        # Apply penalties from info buffer (e.g., oscillation penalties)
        if 'oscillation_penalty' in self.info_buffer:
            # Apply oscillation penalty at full severity without reward scaling
            # to ensure it has significant impact regardless of asset size
            oscillation_penalty = self.info_buffer['oscillation_penalty']
            reward += oscillation_penalty  # Apply directly without scaling
            logger.info(f"Applied oscillation penalty: {oscillation_penalty:.6f}")
            # Clear for next step
            del self.info_buffer['oscillation_penalty']
        
        # Apply extreme penalty for attempting to trade during cooldown
        if attempted_trade_during_cooldown:
            # Track cooldown violation count for progressive penalties
            if not hasattr(self, 'cooldown_violation_count'):
                self.cooldown_violation_count = 0
            self.cooldown_violation_count += 1
            
            # Scale penalty based on violation count - extremely severe for repeat offenders
            base_penalty = -20.0 * end_total_asset * self.reward_scaling  # Doubled the base penalty
            scaled_penalty = base_penalty * min(10, self.cooldown_violation_count)  # Increased maximum multiplier
            reward += scaled_penalty
            logger.warning(f"Applied severe cooldown violation penalty: {scaled_penalty:.6f} (violation #{self.cooldown_violation_count})")
        else:
            # Reset violation count after compliance period
            if hasattr(self, 'cooldown_violation_count') and self.cooldown_violation_count > 0:
                if not hasattr(self, 'compliance_counter'):
                    self.compliance_counter = 0
                self.compliance_counter += 1
                
                # Reset violation count after substantial compliance period
                if self.compliance_counter > 5000:  # Increased from 1000 to 5000
                    prev_count = self.cooldown_violation_count
                    self.cooldown_violation_count = 0
                    self.compliance_counter = 0
                    logger.info(f"Reset cooldown violation count from {prev_count} to 0 after compliance period")
...
class DimensionAdjustingVecEnv(DummyVecEnv):
    ...
    def __init__(self, env_fns):
        ...
        try:
            ...
            logger.error(f"Error during observation size test: {e}")
    
    def _save_obs(self, env_idx, obs):
        """
        Store an observation from env_idx into the observation buffer.
        Handle cases where observation dimensions don't match the buffer.
    ...
    def reset(self):
        """
        Reset all environments and return the resulting observation.
        Handles both old-style and new-style reset methods.
        
        Returns:
            numpy.ndarray: Initial observations from all environments
        """
        # Set up reset options
        reset_kwargs = {}
        
        for env_idx in range(self.num_envs):
            # Reset each environment with the appropriate seed
            try:
                ...
                # Handle return format (obs or (obs, info))
                if isinstance(obs, tuple) and len(obs) == 2:
                    obs, _ = obs  # Ignore the info dictionary
            except TypeError:
...
def train_with_finrl(
    ...
    if net_arch is None:
        ...
        if model_name.lower() in ['a2c', 'ppo']:
            ...
            net_arch = [
                dict(
                    pi=[512, 512, 256],  # Increased policy network capacity
                    vf=[512, 512, 256]   # Increased value function capacity
                )
            ]
        else:
            # For Q-learning based methods like SAC
            net_arch = [512, 512, 256, 128]  # Deeper network architecture
    ...
    policy_kwargs = {
        ...
        'features_extractor_kwargs': {
            'features_dim': 512  # Increase feature dimension for better representation
        }
    }
    ...
    if model_name.lower() == 'ppo':
        model_kwargs.update({
            ...
            'max_grad_norm': 0.5  # Lower gradient norm to stabilize training
        })
    elif model_name.lower() in ['sac', 'td3', 'ddpg']:
        # Smooth the policy updates and add regularization
        model_kwargs.update({
            'learning_rate': 1e-4,  # Lower learning rate for stability
            'buffer_size': 200000,  # Larger buffer for better experience retention
            'learning_starts': 20000,  # More steps before learning starts
            'batch_size': 256,
            'train_freq': 2,  # Train less frequently
            'gradient_steps': 1,
            'action_noise': None,
            'replay_buffer_class': None,
            'replay_buffer_kwargs': None,
            'optimize_memory_usage': False,
            'tau': 0.002,  # Slower target network update for stability
            'gamma': 0.98,  # Slightly lower discount factor to focus on shorter-term rewards
            'use_sde': False
        })
        ...
        if model_name.lower() == 'sac':
            model_kwargs.update({
                ...
                'target_entropy': 'auto',
                # Add additional params for stability
                'target_update_interval': 4,  # Less frequent target updates
...
def create_parallel_finrl_envs(df, args, num_workers=4):
    ...
    if use_subproc and (device is None or str(device).startswith('cpu')):
        ...
        if use_subproc and device is not None and not str(device).startswith('cpu'):
            logger.info("SubprocVecEnv requested but running on GPU. Using DimensionAdjustingVecEnv instead for better GPU utilization.")
        # Use DimensionAdjustingVecEnv instead of DummyVecEnv for better observation handling
        vec_env = DimensionAdjustingVecEnv(env_list)
        logger.info("Using DimensionAdjustingVecEnv for environment vectorization")
